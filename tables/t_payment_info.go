...
type PaymentInfo struct {
	...
	PayStatus    int       `gorm:"column:pay_status;type:smallint(6);default:0;comment:支付状态（0：未支付；1：支付确认中；2：支付成功；3：支付失败；4：支付过期）;NOT NULL" json:"pay_status"`
	RefundHash   string    `gorm:"column:refund_hash;type:varchar(255);comment:退款交易hash;NOT NULL" json:"refund_hash"`
	RefundStatus int       `gorm:"column:refund_status;type:smallint(6);default:0;comment:退款状态：（ 0：无需退款，1，等待退款，2：退款中，3：退款完成）;NOT NULL" json:"refund_status"`
	CancelStatus int       `gorm:"column:cancel_status;type:smallint(6);default:0;comment:订单取消标志（0：未取消，1：取消）;NOT NULL" json:"cancel_status"`

...
func (h *HttpHandle) DistributionList(ctx *gin.Context) {
	...
	if err = h.doDistributionList(ctx.Request.Context(), &req, &apiResp); err != nil {
		log.Error("doDistributionList err:", err.Error(), funcName, clientIp, remoteAddrIP, ctx.Request.Context())
	}
	ctx.JSON(http.StatusOK, apiResp)
}

func (h *HttpHandle) doDistributionList(ctx context.Context, req *ReqDistributionList, apiResp *api_code.ApiResp) error {
